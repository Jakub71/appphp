    ...
    function __construct(){
        ...
        if(isset($_COOKIE[$this->CookieName]) && !$this->id){
            ...
            if($this->login($c['login'], $c['haslo'], false, true)){
                $this->kom[] = "Automatyczne logowanko";
                }
        }
    ...
    function login($login, $haslo, $rem=false, $load=true){
        if ($load && $this ->is_user($login, $haslo)){
            if ($rem) {
                ...
                if($a) $this->kom[] = 'Zapisano pieguski';
                $this->kom[] = "Witaj samuraju $login.";
                return true;
            }
        }else{
            $this->kom[]='<p class="text-warning">Rączki do góry</p>';
            return false;
        }
        if ($rem){
